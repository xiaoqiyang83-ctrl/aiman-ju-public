...
- **每个镜头的visual_prompt必须完整填写所有6个字段**：lighting（含色温K值）、color_palette（含hex色值）、character_placement（角色位置）、facial_detail（面部表情）、scene_description（含场景地点）、composition（构图法则）。任何一个字段都不能为空，不能省略
- **每个镜头的action_prompt必须完整填写**：physical_action（物理级动作描述）和micro_movement（微动作细节），不能为空
- **每个镜头的emotion_cue必须完整填写**：primary_emotion（主要情绪）和visual_mapping（视觉映射），不能为空`;
...
function compileImagePrompt(shot, scene, characters) {
    ...
    // 3. 从visual_prompt结构化字段提取（v7.0.8 优化字段顺序：位置>描述>其他）
